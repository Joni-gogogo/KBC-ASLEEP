...
class Experiment:
    ...
    def eval_relation_prediction(self, model: ASLEEP, binarizer, triples):
    ...
    def train_and_eval(self):
        ...
        model = ASLEEP(settings=self.settings, num_entities=len(self.entity_idx), num_relations=y.shape[1])
        self.logger.info('ASLEEP starts training.')
        model.fit(X, y)
